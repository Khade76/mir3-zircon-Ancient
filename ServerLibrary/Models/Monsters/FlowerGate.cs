...
{
    public class FlowerGate : MonsterObject
    {
        ...
        protected override void OnSpawned()
        {
            ...
            foreach (SConnection con in SEnvir.AuthenticatedConnections)
                con.ReceiveChat(string.Format(con.Language.FlowerGateOpen, CurrentMap.Info.Description, CurrentLocation), MessageType.System);
        ...
        public override void Process()
        {
            ...
            if (SEnvir.Now >= DespawnTime)
            {
                ...
                foreach (SConnection con in SEnvir.AuthenticatedConnections)
                    con.ReceiveChat(con.Language.FlowerGateClosed, MessageType.System);
            ...
            if (SEnvir.Now >= SearchTime && SEnvir.FlowerMapRegion != null && SEnvir.FlowerMapRegion.PointList.Count > 0)
            {
                SearchTime = SEnvir.Now.AddSeconds(3);
                Map map = SEnvir.GetMap(SEnvir.FlowerMapRegion.Map);
                ...
                for (int i = CurrentMap.Objects.Count - 1; i >= 0; i--)
                {
                    ...
                    switch (ob.Race)
                    {
                        case ObjectType.Player:
                            ...
                            ob.Teleport(map, SEnvir.FlowerMapRegion.PointList[SEnvir.Random.Next(SEnvir.FlowerMapRegion.PointList.Count)]);
                            break;
                        default:
                            continue;
                    }
